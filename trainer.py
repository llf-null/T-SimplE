    ...
    def train(self, early_stop=False):
        ...
        for epoch in range(1, self.params.ne + 1):
            ...
            while not last_batch:
                optimizer.zero_grad()
                heads, rels, tails,dates = self.dataset.nextBatch(self.params.bsize,self.model_name,neg_ratio=self.params.neg_ratio)
                last_batch = self.dataset.wasLastBatch()
                scores=self.model(heads,rels,tails,dates)
