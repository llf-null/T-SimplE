...
parser.add_argument('-model', help='Model', type=str, default='T_complex', choices = ['T_complex','T_simple','T_distmult'])
...
parser.add_argument('-neg_ratio', help='Negative ratio', type=int, default=5, choices = [5])#500
parser.add_argument('-dropout', help='Dropout probability', type=float, default=0.4, choices = [0.0, 0.2, 0.4])
parser.add_argument('-save_each', help='Save model and validate each K epochs', type=int, default=100, choices = [100])#20
parser.add_argument('-se_prop', help='Static embedding proportion', type=float, default=0.36)

args = parser.parse_args()

dataset = Dataset(args.dataset,args.model)
